...
helpers.sendTwilioSms = function(phone, msg, callback){
  ...
  if(phone && msg){
    ...
    // configure request details
      ...
      // bind to the error event so it doesnt get thrown (it owuld kook the thread)
...
helpers.getStaticAssets = function(fileName, callback){
  ...
  if(fileName){
    ...
    fs.readFile(publicFid + fileName, function(err, data){
      if(!err && data){
        callback(false, data)
      } else {
        calllback('No file could be found.')
      };
    });
  } else {
    callback('Error, a vlid file name was not specified');
  }
};
