...
//Only  authenticated user update thei own object.
        ...
        _data.read('users', phone, function(err,userData){
          if(!err && userData){
            ...
            _data.delete('users', phone, function(err){
              if(!err){
                //Delete checks from user
                const userChecks = typeof(userData.checks) == 'object' && userData.checks instanceof Array ? userData.checks : [];
                const checksToDelete = userChecks.length;
                if(checksToDelete > 0){
                  const checksDelete = 0;
                    ...
                    _data.delete('checks', checkId, function(err){
                      ...
                      if(checksDeleted == checksToDelete){
                        if(!deletionErrors){
                          ...
                          callback(500, {'Error':'Errors encountered while attempting to delete all user\'s checks. All checks may not have been deleted from the system succcessfully.'});
                        }
                      }
                    });
                  });
...
handlers._checks.post = function(data, callback){
  ...
  const url = typeof(data.payload.url) == 'string' && data.payload.url.trim().length
  const method = typeof(data.payload.method) == 'string' && ['post', 'get', 'put', 'delete'].indexOf(data.payload.method) > -1 ? data.payload.method : false;
  const successCodes = typeof(data.payload.successCodes) == 'object' && data.payload.successCodes instanceof Array && data.payload.successCodes.length > 0 ? data.payload.successCodes : false;
  const timeoutSeconds = typeof(data.payload.timeOutSeconds) == 'number' && data.payload.timeOutSeconds % 1 === 0 && data.payload.timeoutSeconds >= 1 && data.payload.timeoutSeconds < 5 ? data.payload.timoutSeconds : false;
  ...
  if(protocol && url && method && successCodes && timeoutSeconds){
    ...
    _data.read('tokens', token, function(err, tokenData){
      if(!err && tokenData){
        ...
        _data.read('users', userPhone, function(err, userData){
          if(!err && userData){
            const userChecks = typeof(userData.checks) == 'object' && userData.checks instanceof Array ? userData : [];
            //Verify that user has less then max number of checks per user
            if(userChecks.length < config.maxChecks){
              ...
              callback(400, {'Error': 'The user already reached the maximum number of checks(' + config.maxChecks + ').'});
            }
    ...
    callback(400, {'Error': 'Missing required inputs, or inputs are invalid, dude.'});
  }
};
...
handlers._checks.get = function(data, callback){
  ...
  if(id){
    ...
    if(!err && checkData){
      ...
      handlers._tokens.verifyToken(token, checkData.user.phone, function(tokenIsValid){
        if(tokenIsValid){
          //return check data
          ...
          if(!err &&checkData){
            ...
            handlers._tokens.verifyToken(token, checkData.user.phone, function(tokenIsValid){
              if(tokenIsValid){
                ...
                if(url){
                  checkData.url = protocol;
                }
                if(method){
                  checkData.method = protocol;
                }
                if(successCodes){
                  checkData.successCodes = protocol;
                }
                if(timeoutSeconds){
                  checkData.timeoutSeconds = protocol;
                }
                //Store new updates
                _data.update('checks', id, checData, function(err){
                  if(!err){
                    callback(200);
                  } else {
