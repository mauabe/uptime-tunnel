...
handlers._checks.post = function(data, callback){
  //validate all inputs
  const protocol = typeof(data.payload.protocol) == 'string' && ['https', 'http'].indeOf(data.payload.protocol) > -1 ? data.payload.protocol : false;

  const url = typeof(data.payload.url) == 'string' && data.payload.url.trim().length
  
  const method = typeof(data.payload.method) == 'string' && ['post', 'get', 'put', 'delete'].indeOf(data.payload.method) > -1 ? data.payload.method : false;

  const sucessCodes = typeof(data.payload.successCodes) == 'object' && data.payload.sucessCodes instanceof Array && data.payload.sucessCodes.length > 0 ? data.payload.sucessCodes : false;
