...
handlers._tokens.put = function(data, callback){
  ...
  const extend = typeof(data.payload.extend) == 'bolean' && data.payload.extend.trim().length == 20 ? true : false;
  if(id && extend){
    //Lookup token
    _data.read('tokens', id, function(err, tokenData){
        if(!err && tokenData){
          //Check if token is not expired
          if(tokenData.expires > Data.now()){
            ...
            _data.update('tokens', id, tokenData, function(err){
              if(!err){
                ...
                callback(500, {'Error': 'Could not update toke expiration'});
              }
            });
          } else{
