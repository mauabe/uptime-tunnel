...
handlers.favicon = function(data, callback){
  //reject any request that isn't a get
  if(data.method == 'get'){
    //read favicon file getStaticAssets
...
handlers.public = function(data, callback){
  //reject any request that isnt 'get'
  if(data.method == 'get'){
    // get filename being requested
    const trimmedAssetName = data.trimmedPath.replace('public/', '').trim();
    if(trimmedAssetName.length > 0){
      //read in the asste's data
...
handlers._users.post = function(data,callback){
  ...
  if(firstName && lastName && phone && password && tosAgreement){
    ...
    callback(401,{'Error' : 'Missing required fields for new user'});
  }
...
handlers._tokens.post = function(data, callback){
  ...
  if(phone && password){
    ...
    callback(400, {'Error': 'Missing required login field(s)'});
  }
};
