...
handlers.index = function(data, callback){
        //callback(undefined, undefined, 'html');  //reject any request taht isnt a GET
  if(data.method == 'get'){
    //prepare data for interpolation
    var templateData= {
      'head.title' : 'Uptime Tunnel monitoring ',
      'head.description':'Simple uptime monitoring for HTTP/HTTPS for any website. When the website you are monitoring goes down, Uptime Tunnel will send you a text message alert.',
      'body.class': 'index'
    };
...
handlers.accountCreate = function(data,callback){
  ...
  if(data.method == 'get'){
    ...
    var templateData = {
      ...
      'head.description':'Sign up is easy and only take a few moments. We do require a valid phone able to receive text message notifications.',
...
handlers._users.post = function(data,callback){
  ...
  if(firstName && lastName && phone && password && tosAgreement){
    ...
    callback(400,{'Error' : 'Missing required new user account fields'});
  }
  ...
  // Users - get
  //Required data: phone      //Optional data: none     //Only allow authenticated user to access its own data
...
//Users - put   
// Required data: phone  //Optional data: fistName, lastName, password (at least one must be specified)   //Only  authenticated user update their own object.
...
//Users - delete
//Required fied: phone   // Only let authenticated user modify its owon object   // Cleanup (delete) any other data files associate with this user
        ...
        _data.read('users',userPhone,function(err,userData){
          if(!err && userData){
            ...
            if(userChecks.length < config.maxChecks){
              ...
              //verify that URL give had DNS entries, an therefore can resolve 
              ...
              dns.resolve(hostName, function(err, records){
                if(!err && records){
                  ...
                  callback(400, {'Error':'The hostname of the URL enderes did not resolve to any DNS entries'})
                }
              })
            } else {
