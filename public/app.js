...
app.client.request = function(headers, path, method, queryStringObject, payload, callback) {
  ...
  for (let queryKey in queryStringObject) {
    if (queryStringObject.hasOwnProperty(queryKey)) {
      counter++;
      // preprend new query strings with  ampersand
      if (counter > 1) {
        requestUrl += '&';
      }
        ...
        for (let i = 0; i < elements.length; i++) {
          if (elements[i].type !== 'submit') {
            ...
            if (nameOfElement == '_method') {
              ...
              // Create payload field "method" if elements name is httpmethod
              if (nameOfElement == 'httpmethod') {
                nameOfElement = 'method';
              }
              // Create payload field named "id" if elements name is uid
              if (nameOfElement == 'uid') {
                nameOfElement = 'id';
              }
              // If element has class "multiselect" add its value(s) as array elements
...
app.formResponseProcessor = function(formId, requestPayload, responsePayload) {
  ...
  // If login was successful, set token in localstorage and redirect user
...
// Get session token from local storage and set it in app.config object
  ...
  if (phone) {
    ...
    app.client.request(undefined, 'api/users', 'GET', queryStringObject, undefined, function(
      ...
      if (statusCode == 200) {
        ...
        // If request is not 200, log the user out
        app.logUserOut();
      }
    });
  } else {
...
app.loadChecksListPage = function() {
  // Get phone number from the current token, or log the user out
  let phone =
    typeof app.config.sessionToken.phone == 'string' ? app.config.sessionToken.phone : false;
  if (phone) {
    // Fetch  user data
    ...
    app.client.request(undefined, 'api/users', 'GET', queryStringObject, undefined, function(
      ...
      if (statusCode == 200) {
        ...
        if (allChecks.length > 0) {
          // Show each new check as new row in table
          allChecks.forEach(function(checkId) {
            ...
            app.client.request(
              ...
              function(statusCode, responsePayload) {
                if (statusCode == 200) {
                  let checkData = responsePayload; // NOT USED??
                  // Make check data into a table row
...
app.loadChecksEditPage = function() {
  // Get check id from query string, if none is found then redirect back to dashboard
  let id =
  ...
  if (id) {
    ...
    app.client.request(undefined, 'api/checks', 'GET', queryStringObject, undefined, function(
      ...
      if (statusCode == 200) {
        ...
        // If request is not 200, redirect back to dashboard
        window.location = '/checks/all';
      }
    });
  } else {
...
// Call init processes after the window loads
